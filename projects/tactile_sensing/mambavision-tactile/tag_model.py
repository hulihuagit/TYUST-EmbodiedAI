...
class MambaVisionEncoderWrapper(nn.Module):
    ...
    def forward_features_with_spatial(self, x):
        global_feat, _outs = self.model.forward_features(x)
        spatial_map = _outs[-1]

        if spatial_map.dim() == 4:
            B, C, H_f, W_f = spatial_map.shape
    ...
    def forward_features_multi(self, x):
        ...
        if stage3_map.dim() == 4:
            B, C, H, W = stage3_map.shape
            stage3 = stage3_map.flatten(2).transpose(1, 2)
        else:
            stage3 = stage3_map

        if stage4_map.dim() == 4:
            B, C, H, W = stage4_map.shape
...
class HierarchicalTactileAttention(nn.Module):
    ...
    def forward(self, v3, v4, tactile):
        ...
        fused = torch.cat([v3_feat, v4_feat], dim=-1)
...
class DynamicCrossModalFusion(nn.Module):
    ...
    def forward(self, v_feat, t_feat, v_global_ctx, t_global_ctx):
        ...
        gate_input = torch.cat(
            [v_feat, t_feat, v_global_ctx, t_global_ctx], dim=-1
        )
...
class TemporalAttentionPooling(nn.Module):
    def __init__(self, d_model, dropout=0.2):
        super().__init__()
        self.attn_net = nn.Sequential(nn.Linear(d_model, d_model // 4), nn.Tanh(), nn.Dropout(dropout), nn.Linear(d_model // 4, 1))
...
class DynamicTwoStreamTemporalFusion(nn.Module):
    ...
    def compute_global_context(self, seq):
        return seq.mean(dim=1) + seq.std(dim=1)

    def forward(self, v_seq, t_seq):
        B, T, D = v_seq.shape
        v_temporal = self.vision_temporal_mamba(self.vision_norm(v_seq))
        v_temporal = v_seq + self.dropout(v_temporal)
        t_temporal = self.tactile_temporal_mamba(self.tactile_norm(t_seq))
        t_temporal = t_seq + self.dropout(t_temporal)
        ...
        for i in range(T):
            fused_t, gate_t = self.cross_modal_fusion(v_temporal[:, i, :], t_temporal[:, i, :], v_global_ctx, t_global_ctx)
        ...
        analysis_info = {'temporal_attention': temporal_attn, 'dynamic_gates': gate_seq}
        return pooled, analysis_info
...
class SpatioTemporalDynamicClassifier(nn.Module):
    def __init__(
        ...
        self.v_spatial_proj = nn.Sequential(nn.Linear(encoder_out_dim, d_model), nn.LayerNorm(d_model))
        self.t_proj = nn.Sequential(nn.Linear(encoder_out_dim, d_model), nn.LayerNorm(d_model))

        if hierarchical:

            self.v3_spatial_proj = nn.Sequential(nn.Linear(encoder_out_dim_s3, d_model), nn.LayerNorm(d_model))
            self.spatial_attention = HierarchicalTactileAttention(d_model, num_heads=8, dropout=dropout)
        else:
            self.spatial_attention = TactileGuidedSpatialAttention(d_model, num_heads=8, dropout=dropout)

        self.temporal_fusion = DynamicTwoStreamTemporalFusion(d_model=d_model, d_state=d_state, dropout=dropout)
        self.classifier = nn.Sequential(
            nn.Linear(d_model, d_model // 2), nn.GELU(), nn.Dropout(dropout), nn.Linear(d_model // 2, num_classes)
        )

    def forward(self, rgb, tac, return_analysis=False):
        assert rgb.ndim == 5, "需要时序输入 [B, T, 3, H, W]"
        b, t_len, c, h, w = rgb.shape

        rgb_flat = rgb.reshape(b * t_len, c, h, w)
        tac_flat = tac.reshape(b * t_len, c, h, w)

        t_global_flat = self.tactile_encoder.forward_features(tac_flat)
        ...
        if self.hierarchical:
            ...
            v_attended_flat, heat3_flat, heat4_flat = self.spatial_attention(v3_flat, v4_flat, t_global_flat)
            spatial_heatmaps = {
            ...
            v_attended_flat, spatial_heatmaps_flat = self.spatial_attention(v_spatial_flat, t_global_flat)
            spatial_heatmaps = spatial_heatmaps_flat.reshape(b, t_len, -1)
