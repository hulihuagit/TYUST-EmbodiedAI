...
def get_args_parser():
    ...
    #HRNet
    #parser.add_argument('--num-joints', default=256, type=int, help='num_joints')
    return parser
...
def main(args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        checkpoint_latest_path = os.path.join(args.checkpoints_dir, 'latest_hr25_8patch_d8_0117.pth')
        torch.save({
        ...
        if epoch % args.eval_freq == 0 and epoch != 0:
            ...
            if abs(np.min(mae) - result[0]) < 0.01:
                checkpoint_best_path = os.path.join(args.checkpoints_dir, 'best_mae_hr25_4patch_d8_0117.pth')
                torch.save({
                    'model': model_without_ddp.state_dict(),
